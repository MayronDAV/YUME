...
class PremakeConfiguration:
	...
	premakeDirectory = f"./Thirdparty/premake/bin/{system}"
	...
	def CheckIfPremakeInstalled(cls):
		...
		if (cls.system == "linux"): premakeExe = Path(f"{cls.premakeDirectory}/premake5");
	...
	def InstallPremake(cls):
		...
		premakePath = f"{cls.premakeDirectory}/premake-{cls.premakeVersion}-{cls.system}.{cls.extension}"
